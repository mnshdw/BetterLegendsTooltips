	...
	q.getTooltip = @(__original) function() {
		...
		foreach(index, entry in tooltip) {
			if ("type" in entry && entry.type == "text" && "text" in entry && "icon" in entry) {
				...
				// If CollapseNamedEnemies is enabled, determine the base name of the entity
				if (::ModBetterLocationTooltips.CollapseNamedEnemies && "icon" in entry && typeof entry.icon == "string") {
		...
		foreach(key, group in groupedEntities) {
			...
			if (group.count > 1) {
				entry.text = group.count + " [color=800808]Champion[/color] " + entry.text + "s";
			}
